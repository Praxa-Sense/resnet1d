...
if __name__ == "__main__":
    ...
    batch_size = 32
    writer = SummaryWriter('runs/challenge2017/crnn')
    ...
    model.to(device)
    # summary(model, (X_train.shape[1], X_train.shape[2]), device=device)
    ...
    step = 0
    prev_f1 = 0
    for ep in tqdm(range(n_epoch), desc="epoch", leave=False):
        ...
        print(confusion_matrix(Y_test, all_pred, normalize='all'))
        f1_score = (tmp_report['0']['f1-score'] + tmp_report['1']['f1-score'])/2
        if f1_score > prev_f1:
            print(ep, f1_score)
            os.makedirs('models/crnn', exist_ok=True)
            torch.save(model, 'models/crnn/crnn_{}_{:.4f}.pt'.format(ep, f1_score))
            prev_f1 = f1_score
